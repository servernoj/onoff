        ...
        setTimeout(() => {
            ...
            if (status === 1) {
                if (DEBUG) {
                    const mem = process.memoryUsage();
                    try {
                        const log = `[onoff:worker] events=${eventCount}, heapMB=${Math.round(mem.heapUsed / 1024 / 1024)}, rssMB=${Math.round(mem.rss / 1024 / 1024)}, externalMB=${Math.round((mem.external || 0) / 1024 /
                            1024)}`;
                        appendFileSync('/tmp/onoff-worker-events.log', `${Date.now()}: ${log}\n`);
                    } catch { }
                }
                eventCount += 1;
                const value = gpiod.getLineValue(line);
